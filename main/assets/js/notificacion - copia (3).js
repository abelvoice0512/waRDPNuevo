...
function transferirFila(tr, token){
	...
	if (i!=-1){
		...
		$(tr).attr("data-dsc-respuesta","No se intentó la notificacion ya que SAP devolvio anteriormente error para el producto: " + CodProducto + " en el grupo: " + CodGrupo);
				...
				if (CodigoRespuesta==300){		//token caduco o no es valido
					...
					if (CodGrupo=="Molienda de Cemento" || CodGrupo=="Molienda de Crudo" || CodGrupo=="Chancado Secundario"){
						if (CodigoRespuesta.trim()!="0"){
							...
							anularFilasGrupoProducto(CodGrupo, CodProducto, token);
						}
						else {
							//actualizamos % de avance
							var porc = (parseFloat(localStorage["nFila"])+1)/parseFloat(localStorage["nFilas"])*100.00;
...
function anularFilasGrupoProducto(grupo, producto, token){
	...
	if ($("tr.transferir[data-estado='T'][data-grupo='"+grupo+"'][data-producto='"+producto+"']").length >0){
		anularFila($("tr.transferir[data-estado='T'][data-grupo='"+grupo+"'][data-producto='"+producto+"']")[0], grupo, producto, token)
	}
	else {
...
function anularFila(tr, grupo, producto, token){
	...
	$.ajax({ 
		...
		success: function(data){	
			...
			{	
				...
				//mostramos icono de error
				$(tr).find("td.estado .transf-en-progreso").addClass("d-none");
				$(tr).find("td.estado .transf-transferido").addClass("d-none");
				$(tr).find("td.estado .transf-error").removeClass("d-none");

				//seteamos los resultados en los atributos de la fila producto
				$(tr).attr("data-cod-respuesta",data.CodigoRespuesta.trim());
				$(tr).attr("data-dsc-respuesta","Se eliminó la notificación ya que el producto tiene error para otro envio en el grupo: " + grupo);
				$(tr).attr("data-estado","E");
				$(tr).attr("data-numero-notificacion","")
				$(tr).attr("data-numero-contador","");
				$(tr).attr("data-dsc-respuesta-detalle",data.MensajeDetalle.trim());
				...
				//seteamos los resultados en los atributos de la fila producto
				$(tr).attr("data-cod-respuesta",data.CodigoRespuesta.trim());
				$(tr).attr("data-dsc-respuesta","Se eliminó con errores la notificación ya que el producto tiene error para otro envio en el grupo: " + grupo + ". " + data.Mensaje.trim() );
				$(tr).attr("data-estado","E");
				$(tr).attr("data-numero-notificacion","")
				$(tr).attr("data-numero-contador","");
				$(tr).attr("data-dsc-respuesta-detalle",data.MensajeDetalle.trim());
