...
function configuracionInicial() {
	...
	*/
	...
	$("#lbl-entorno-sap2").text(parametros.system_id_sap + " - " + parametros.client_sap);
...
function configuracionEventos(){
	...
	$("#btn-transferir").on("click", function(){
		...
		//validamos si es dia actual para preguntar al usuario si desea notificar un dia que aun no cerró
		var cFechaReporte = $("#fecha-reporte").val();
		var dFechaActual = new Date();
		var cFechaActual = obtener_cadena_fecha_estandar(dFechaActual);

		if (cFechaReporte<cFechaActual) {
			...
			//el dia actual no ha cerrado, preguntamos al usuario si desea notificar de todas maneras
			mostrarModal("modal-transferir-dia-actual");
		}	
	});
	...
	$("#btn-aceptar-modal-transferir-dia-actual").on("click", function(){
		ocultarModal("modal-transferir-dia-actual");
	...
	$("#btn-cancelar-modal-transferir-dia-actual").on("click", function(){
		ocultarModal("modal-transferir-dia-actual");
	});
	...
	$("#btn-aceptar-modal-resumen").on("click", function(){
		ocultarModal("modal-resumen");
	});

	$("#btn-cancelar-modal-eliminar-notificacion").on("click", function(){
		ocultarModal("modal-eliminar-notificacion");
	});

	$("#btn-cancelar-modal-login-eliminar-notificacion").on("click", function(){
		ocultarModal("modal-login-eliminar-notificacion");
	});
}
...
var nFilas=0;
var nFila=0;
var grupo_producto_bloqueados = [];

function transferir(token){
	
	localStorage["nFilas"] = $("tr.transferir.ok.sel").length;
	localStorage["nFila"] = 0;
	grupo_producto_bloqueados=[];

	if ($("tr.transferir.ok.sel").length>0){	
		...
		$.ajax({ 
			...
			beforeSend: function (){            
				...
				//bloqueamos botones de eliminacion
				$(".eliminar-notificacion").attr("disabled","disabled");

				//bloqueamos checkboxs de seleccion
				$(".seleccionar-producto").attr("disabled","disabled");
	        }, 
	        success: function(data){
	        	if (data.codigo==0){
					...
					//mostramos el spinner en las filas de productos
					$("tr.transferir.ok.sel td.estado i").addClass("d-none");
					$("tr.transferir.ok.sel td.estado .transf-en-progreso").removeClass("d-none");
					...
					transferirFila($("tr.transferir.ok.sel")[0], token);	
				}
				else {
					...
					//activamos el boton transferir y ocultamos su spinner
					$("#btn-transferir").removeAttr("disabled")
					$("#btn-transferir .spinner-border").addClass("d-none");

					//activamos botones de eliminacion
					$(".eliminar-notificacion").removeAttr("disabled");

					//activamos checkboxs de seleccion
					$(".seleccionar-producto").removeAttr("disabled");
...
function transferirFila(tr, token){
	...
	var CodGrupo = aId[0];
	var CodMaquina = aId[1];
	var CodProducto = aId[2];

	//validamos datos
	var i = grupo_producto_bloqueados.findIndex(x => x.grupo==CodGrupo && x.producto==CodProducto)
	if (i!=-1){
		//la combinacion grupo-producto esta bloqueada para notificacion
		...
		var CodMaquinaBloqueada = grupo_producto_bloqueados[i].maquina;
		$(tr).attr("data-cod-respuesta","-1");
		$(tr).attr("data-dsc-respuesta","No se intentó la notificacion ya que SAP devolvio anteriormente error para el producto: " + CodProducto + " en la máquina " + CodMaquinaBloqueada + " y el grupo: " + CodGrupo);
		$(tr).attr("data-estado","E");
		$(tr).attr("data-dsc-respuesta-detalle","");
		$(tr).attr("data-intentos",1);
		...
		if(parseInt(localStorage["nFilas"])<parseInt(localStorage["nFila"])+1){
			...
			//removemos class sel a checkboxes de seleccion sin check
			$(".seleccionar-producto:not(:checked)").parent().parent().removeClass("sel");

			//removemos class ok a bloques que pasaron con exito
			//$("tr.transferir.ok[data-estado='T']").removeClass("ok");
		}
		else {
			transferirFila($("tr.transferir.ok.sel")[parseInt(localStorage["nFila"])], token);
		}
		

	}	
	else if (NumOP===undefined || NumOP.trim()=="" || NumOP.trim()=="null"){
...
function ObtenerJSONTotal(){
	...
	for (i=0; i<$("tr.transferir.ok.sel").length; i++){
		var tr = $("tr.transferir.ok.sel")[i];
		...
		$.ajax({ 
			...
			success: function(data){	
				...
				if(data.CodigoRespuesta.trim()=="0"){
					...
					$(tr).attr("data-dsc-respuesta-detalle","");
					$(tr).attr("data-intentos",localStorage["nIntento"]);
					
					//agregamos boton eliminar
					$(tr).find(".eliminar-notificacion").remove();
					var htmlBotonEliminar = '<button type="button" class="btn btn-icon btn-sm js-sweetalert eliminar-notificacion" title="Eliminar" data-type="confirm"><i class="fa fa-trash-o text-danger"></i></button>';
					$(tr).find(".td-eliminar-notificacion").html(htmlBotonEliminar);

					//bloqueamos boton de eliminar
					$(tr).find(".eliminar-notificacion").attr("disabled","disabled");
					

					//actualizamos evento click en los botones eliminar
					$(".eliminar-notificacion").off("click");
					$(".eliminar-notificacion").on("click", function(){
		        		mostrarModal("modal-eliminar-notificacion");

		        		var NumOP = $(this).parent().parent().attr("data-numero-op");
			        	var NumNotificacion = $(this).parent().parent().attr("data-numero-notificacion");
			        	var NumContador = $(this).parent().parent().attr("data-numero-contador");
			        	var trEliminar = $(this).parent().parent();

		        		$("#btn-aceptar-modal-eliminar-notificacion").off("click");
		        		$("#btn-aceptar-modal-eliminar-notificacion").on("click", function(){
			
							//validamos si existe cookie de token
							if (getCookie("token")!=""){
								//validamos token para eliminar notificacion
								validarTokenParaEliminacionDeNotificacion(NumOP, NumNotificacion, NumContador, getCookie("token"), trEliminar);
							}
							else {
								ocultarModal("modal-eliminar-notificacion");
								mostrarModal("modal-login-eliminar-notificacion");
								$("#mensaje-notificacion2").addClass("d-none");

								$("#btn-aceptar-modal-login-eliminar-notificacion").off("click");
								$("#btn-aceptar-modal-login-eliminar-notificacion").on("click", function(){
									esValidoLoginEliminarNotificacion(NumOP, NumNotificacion, NumContador, trEliminar);		
								});	
							}	        		
		        		});	
		        	});

					//quitamos check de seleccion
					$(tr).find(".seleccionar-producto").prop( "checked", false );
				...
				if (CodigoRespuesta==300){		//token caduco o no es valido
					...
					for (i=0; i<$("tr.transferir.ok.sel").length; i++){
						var estado = $($("tr.transferir.ok.sel")[i]).attr("data-estado");

						$($("tr.transferir.ok.sel")[i]).find("td.estado i").addClass("d-none");
						$($("tr.transferir.ok.sel")[i]).find("td.estado .transf-en-progreso").addClass("d-none");
						
						if (estado=="T"){
							$($("tr.transferir.ok.sel")[i]).find("td.estado .transf-transferido").removeClass("d-none");
						}
						else if (estado=="E"){
							$($("tr.transferir.ok.sel")[i]).find("td.estado .transf-error").removeClass("d-none");
						}
						else if (estado=="A"){
							$($("tr.transferir.ok.sel")[i]).find("td.estado .transf-eliminada").removeClass("d-none");
						}
						else {
							$($("tr.transferir.ok.sel")[i]).find("td.estado .transf-pendiente").removeClass("d-none");
						}
					}
					...
					if (CodGrupo=="Molienda de Cemento" || CodGrupo=="Molienda de Crudo" || CodGrupo=="Chancado Secundario"){
						if (CodigoRespuesta.trim()!="0"){
							...
							//marcamos la combinacion grupo-producto para que en el futuro no se notifiquen
							grupo_producto_bloqueados.push({
								"grupo": CodGrupo,
								"maquina": CodMaquina,
								"producto": CodProducto
							});

							console.log("error al notificar para grupo especial: " + CodGrupo + ", maquina: " + CodMaquina + ", producto: " + CodProducto);

							//anulamos productos similares del mismo grupo
							anularFilasGrupoProducto(CodGrupo, CodMaquina, CodProducto, token);
						}
						else {
							//actualizamos % de avance
							var porc = (parseFloat(localStorage["nFila"])+1)/parseFloat(localStorage["nFilas"])*100.00;
			...
			error: function (xhr, textstatus, thrownError) { 
				...
				//reintentos
				localStorage["nIntento"] = parseInt(localStorage["nIntento"]) + 1;
				if (parseInt(localStorage["nIntento"]) <= parametros.nro_reintentos_notificacion){
					NotificarEnSAP(tr, token);
				}
				else {
					...
					//seteamos los resultados en los atributos de la fila producto
					$(tr).attr("data-cod-respuesta",-1);
					$(tr).attr("data-dsc-respuesta","Status:"+textstatus+", Message:"+thrownError);
					$(tr).attr("data-estado","E");
					$(tr).attr("data-dsc-respuesta-detalle","");
					$(tr).attr("data-intentos",localStorage["nIntento"]);
					...
					//mostramos modal de timeout en los servicios
					$("#modal-error .modal-body").html("Error de comunicación con SAP.<br>Status:"+textstatus+", Message:"+thrownError);
...
function insertarNotificacionSQL(tr){
	...
	var DscUnidadGas = $(tr).attr("data-unidad-gas");
	var NumIntentos = $(tr).attr("data-intentos");	
...
function anularFilasGrupoProducto(grupo, maquinaBloqueada, producto, token){

	//buscamos filas grupo-producto que hayan transferido con exito para anularlas
	localStorage["nFilasAnular"] = $("tr.transferir[data-estado='T'][data-grupo='"+grupo+"'][data-producto='"+producto+"']").length;
	localStorage["nFilaAnular"] = 0;

	console.log ("Se encontraron " + localStorage["nFilasAnular"] + " filas por anular para el grupo: " + grupo + ", producto: " + producto);

	if ($("tr.transferir[data-estado='T'][data-grupo='"+grupo+"'][data-producto='"+producto+"']").length >0){
		anularFila($("tr.transferir[data-estado='T'][data-grupo='"+grupo+"'][data-producto='"+producto+"']")[0], grupo, maquinaBloqueada, producto, token)
	}
	else {
		//se termina el proceso de anular / regresamos al proceso de notificaciones
...
function anularFila(tr, grupo, maquinaBloqueada, producto, token){
	...
	$.ajax({ 
		url: parametros.ws_sap+"/RDP.svc/EliminarNotificacion", 
		type: "POST",
		dataType: 'json', 
		headers: {
		    "Content-Type": "application/json",
		...
		success: function(data){	
			...
			if(data.CodigoRespuesta.trim()=="0")
			{	
				console.log("se anulo notificación para el grupo: " + grupo + ", maquina: " + CodMaquina + ", producto: " + producto);

				//mostramos icono de anulado
				$(tr).find("td.estado .transf-en-progreso").addClass("d-none");
				$(tr).find("td.estado i").addClass("d-none");
				$(tr).find("td.estado .transf-eliminada").removeClass("d-none");

				//seteamos los resultados en los atributos de la fila producto
				$(tr).attr("data-cod-respuesta",data.CodigoRespuesta.trim());
				$(tr).attr("data-dsc-respuesta","Se eliminó la notificación ya que el producto tiene error para otro envio en el grupo: " + grupo + " y máquina: " + maquinaBloqueada);
				$(tr).attr("data-estado","A");
				$(tr).attr("data-numero-notificacion","")
				$(tr).attr("data-numero-contador","");
			...
			else if (data.CodigoRespuesta.trim()!="0" && CodigoRespuesta!=300) {

				console.log("se anulo con errores notificación para el grupo: " + grupo + ", maquina: " + CodMaquina + ", producto: " + producto);
				...
				//seteamos los resultados en los atributos de la fila producto
				$(tr).attr("data-cod-respuesta",data.CodigoRespuesta.trim());
				$(tr).attr("data-dsc-respuesta","Se eliminó con errores la notificación ya que el producto tiene error para otro envio en el grupo: " + grupo + " y máquina: " + maquinaBloqueada + ". " + data.Mensaje.trim() );
				$(tr).attr("data-estado","E");
				$(tr).attr("data-numero-notificacion","")
				$(tr).attr("data-numero-contador","");
			...
			if (CodigoRespuesta==300){		//token caduco o no es valido
				...
				for (i=0; i<$("tr.transferir.ok").length; i++){
					...
					if (estado=="T"){
						...
						$($("tr.transferir.ok")[i]).find("td.estado .transf-eliminada").removeClass("d-none");
					}
					else {
				...
				//anulamos siguiente fila
				localStorage["nFilaAnular"] = parseInt(localStorage["nFilaAnular"])+1;
				if(parseInt(localStorage["nFilasAnular"])<parseInt(localStorage["nFilaAnular"])+1){
					...
					anularFila($("tr.transferir[data-estado='T'][data-grupo='"+grupo+"'][data-producto='"+producto+"']")[parseInt(localStorage["nFilaAnular"])], token);
				}
...
function cargarRDP(){
	...
	xhr = $.ajax({ 
		...
		beforeSend: function (){            
			...
			$("#cuerpo-tabla-resumen1").empty();
			$("#cuerpo-tabla-resumen2").empty();
			$("#cuerpo-tabla-resumen3").empty();
			$("#cuerpo-tabla-resumen4").empty();
			$("#cuerpo-tabla-resumen5").empty();
        ...
        success: function(data){ 
        	...
        	$.each(data.oProcesos, function (key, val) {	
        		...
        		$.each(val.MAQUINAS, function (key2, val2) {	
					...
					$.each(val2.PRODUCTOS, function (key3, val3) {	
						...
						var ok="";
						if(transferir!=""){
							if(val3.COD_ESTADO=="" || val3.COD_ESTADO=="E" || val3.COD_ESTADO=="A"){
								ok="ok";
							}
							if(val3.COD_ESTADO=="T"){
								ok="ok";
		                ...
		                if(transferir==""){
		                	...
		                	tr = tr + '		<td class="text-right estado"><i class="far fa-clock transf-pendiente ' + (val3.COD_ESTADO==""?'':'d-none') +'"></i><i class="fas fa-check-double transf-transferido ' + modificado + ' ' + (val3.COD_ESTADO=="T"?'':'d-none') +'"></i><i class="fas fa-exclamation-triangle transf-error ' + (val3.COD_ESTADO=="E"?'':'d-none') +'"></i><i class="fa fa-times-circle transf-eliminada ' + (val3.COD_ESTADO=="A"?'':'d-none') +'"></i><div class="spinner-border transf-en-progreso d-none" role="status"></div></td>';
		                	
		                	if (val3.COD_ESTADO=="T") {
		                		tr = tr + '<td class="td-eliminar-notificacion"><button type="button" class="btn btn-icon btn-sm js-sweetalert eliminar-notificacion" title="Eliminar" data-type="confirm"><i class="fa fa-trash-o text-danger"></i></button></td>';
		                	}
		                	else {
		                		tr = tr + '<td class="td-eliminar-notificacion"></td>';
		                	}

		                	tr = tr + '		<td class="text-right"><input type="checkbox" class="seleccionar-producto"></input></td>';
		                }
						...
						$.each(val3.MATERIALES, function (key4, val4) {	
							...
							$.each(val4.MATERIALES_SAP, function (key5, val5) {	
								...
								if (!(val5.CNT_PORCENTAJE=="0" || val5.CNT_PORCENTAJE=="")){
						    		...
						    		if (val5.CNT_TONELADAS_SAP==null) {
						    			console.log(val5);
						    		}
						    		var nToneladasSAP="";
						    		if (val5.CNT_TONELADAS_SAP!=null) {
						    			nToneladasSAP=val5.CNT_TONELADAS_SAP.replace(/,/gi,'');
						    		}
									...
									if (transferir==""){
							    		...
							    		tr = tr + "			<span>"+val5.CNT_TONELADAS_SAP+"</span>";
							    		tr = tr + "			<i class='fas fa-pencil-alt editar'></i>";
							    		tr = tr + "			<input type='number' class='form-control input-height d-none' style='font-size: 12px;background-color:white; padding-right: 25px;' value='"+ nToneladasSAP +"'/>";
							    		//tr = tr + "			<i class='fas fa-check d-none aceptar' style='position: absolute;right: 27px;top: 50%;transform: translateY(-50%);'></i>";
							    		tr = tr + "			<i class='fas fa-times d-none cancelar' style='position: absolute;right: 13px;top: 50%;transform: translateY(-50%);'></i>";
							    		tr = tr + '		</td>';
									}
						    		...
						    		if (transferir==""){
							    		...
							    		tr2 = tr2 + "			<span>"+nToneladasSAP+"</span>";
							    		tr2 = tr2 + '		</td>';
						    		}
						    		else {
							    		...
							    		tr2 = tr2 + "			<input type='number' class='form-control input-height d-none' style='font-size: 12px;background-color:white; padding-right: 25px;' value='"+ nToneladasSAP +"'/>";
    		...
    		// 1. CALIZAS EN PASE DIRECTO
    		var tr1="";
    		$.each(data.oResumen.oRDPResumenCalizasPaseDirecto, function (key, val) {
    			tr1 = tr1 + '<tr>';
    			tr1 = tr1 + '<td class="text-left">'+val.Nombre+'</td>';
    			tr1 = tr1 + '<td class="text-right">'+val.ValorPD+'</td>';
    			tr1 = tr1 + '<td class="text-right">'+val.ValorOrigen+'</td>';
    			tr1 = tr1 + '</tr>';
    		});

    		$("#cuerpo-tabla-resumen1").html(tr1); 


    		// 2. CHANCADO SECUNDARIO VS APILADO
        	var tr2="";
    		$.each(data.oResumen.oRDPResumenChancadoSecundario, function (key, val) {
    			tr2 = tr2 + '<tr>';
    			tr2 = tr2 + '<td class="text-left">'+val.Nombre+'</td>';
    			tr2 = tr2 + '<td class="text-right">'+val.ValorChancadoSecundario+'</td>';
    			tr2 = tr2 + '<td class="text-right">'+val.ValorApilado+'</td>';
    			tr2 = tr2 + '</tr>';
    		});

    		$("#cuerpo-tabla-resumen2").html(tr2); 


    		// 3. PRODUCTOS Y PORCENTAJES DE MATERIAL
        	var tr3="";
    		$.each(data.oResumen.oRDPResumenProductosPorcentajes, function (key, val) {

    			var produccion = parseInt(val.ValorProduccion.replace(/,/gi,""));
				if (!isNaN(produccion)){
					if (produccion>0){

						tr3 = tr3 + '<tr>';
		    			tr3 = tr3 + '	<td class="text-left">'+val.Proceso+'</td>';
		    			tr3 = tr3 + '	<td class="text-left">'+val.Maquina+'</td>';
		    			tr3 = tr3 + '	<td class="text-left">'+val.Producto+'</td>';
		    			tr3 = tr3 + '	<td class="text-right">'+val.ValorProduccion+'</td>';
		    			tr3 = tr3 + '</tr>';

		    			if (val.oMaterialesPorcentaje.length>0){

		    				tr3 = tr3 + '<tr>';
			    			tr3 = tr3 + '	<td class="" colspan="4">';
			    			tr3 = tr3 + '		 <table class="table table-sm table-hover table-vcenter text-nowrap mb-0 tabla-arbol tabla-sub-resumen">';
			    			tr3 = tr3 + '			<thead>';
			                tr3 = tr3 + '				<tr>';
			                tr3 = tr3 + '					<th class="text-left"></th>';
			                tr3 = tr3 + '					<th class="text-left">Material</th>';
			                tr3 = tr3 + '					<th class="text-right">Porcentaje (%)</th>';
			                tr3 = tr3 + '				</tr>';                
			                tr3 = tr3 + '			</thead>';             
			                tr3 = tr3 + '			<tbody>';

			    			$.each(val.oMaterialesPorcentaje, function (key2, val2) {

			    				tr3 = tr3 + '				<tr>';
			    				tr3 = tr3 + '					<td class="text-left"></td>';
				                tr3 = tr3 + '					<td class="text-left">'+val2.Material+'</td>';
				                tr3 = tr3 + '					<td class="text-right">'+val2.Porcentaje+'</td>';
				                tr3 = tr3 + '				</tr>';
			    			});
    		...
    		$("#cuerpo-tabla-resumen3").html(tr3); 


    		// 4. RESUMEN DE ADITIVOS
        	var tr4="";
    		$.each(data.oResumen.oRDPResumenAditivos, function (key, val) {
    			tr4 = tr4 + '<tr>';
    			tr4 = tr4 + '<td class="text-left">'+val.Aditivo+'</td>';
    			tr4 = tr4 + '<td class="text-right">'+val.ValorTotal+'</td>';
    			tr4 = tr4 + '</tr>';
    		});

    		$("#cuerpo-tabla-resumen4").html(tr4); 


    		// 5. RENDIMIENTO DE PRODUCTOS
        	var tr5="";
    		$.each(data.oResumen.oRDPResumenProductosRendimiento, function (key, val) {
    			tr5 = tr5 + '<tr>';
    			tr5 = tr5 + '<td class="text-left">'+val.Proceso+'</td>';
    			tr5 = tr5 + '<td class="text-left">'+val.Maquina+'</td>';
    			tr5 = tr5 + '<td class="text-left">'+val.Producto+'</td>';
    			tr5 = tr5 + '<td class="text-right">'+val.ValorTH+'</td>';
    			tr5 = tr5 + '<td class="text-left"><i class="fas fa-circle " style="color:'+val.ColorTH+';"></i></td>';
    			tr5 = tr5 + '</tr>';
    		});

    		$("#cuerpo-tabla-resumen5").html(tr5); 


        },
        complete: function(){

        	//seleccionamos los productos con notificacion pendiente, con error o eliminada
        	for (i=0; i<$("tr.transferir.ok").length; i++){
        		var tr = $($("tr.transferir.ok")[i]);
        		var estado = tr.attr("data-estado");

        		if (estado=="E" || estado=="A" || estado==""){
        			tr.find(".seleccionar-producto").prop('checked', true);
        			tr.addClass("sel");
        		}
        	}
        	...
        	$(".estado i").on("click", function(){
        		...
        		var intentos = $(this).parent().parent().attr("data-intentos");
...
function validarTokenParaEliminacionDeNotificacion(NumOP, NumNotificacion, NumContador, token, trEliminar){
	...
	$.ajax({ 
		...
		async: 	true, 
		beforeSend: function (){            
			//bloqueamos boton transferir
			$("#btn-transferir").attr("disabled","disabled");
			
			//bloqueamos botones de eliminacion
			$(".eliminar-notificacion").attr("disabled","disabled");

			//desactivamos botones del modal de eliminar notificacion
			$("#btn-aceptar-modal-eliminar-notificacion").attr("disabled","disabled");
			$("#btn-cancelar-modal-eliminar-notificacion").attr("disabled","disabled");

			//mostramos spinner del boton aceptar
			$("#btn-aceptar-modal-eliminar-notificacion .spinner-border").removeClass("d-none");
        }, 
        success: function(data){
        	//activamos boton transferir
			$("#btn-transferir").removeAttr("disabled");
			
			//activamos botones de eliminacion
			$(".eliminar-notificacion").removeAttr("disabled");

			//activamos botones del modal de eliminar notificacion
			$("#btn-aceptar-modal-eliminar-notificacion").removeAttr("disabled");
			$("#btn-cancelar-modal-eliminar-notificacion").removeAttr("disabled");

			//ocultamos spinner del boton aceptar
			$("#btn-aceptar-modal-eliminar-notificacion .spinner-border").addClass("d-none")

			//ocultamos modal de eliminar notificacion
			ocultarModal("modal-eliminar-notificacion");

        	if (data.codigo==0){
				eliminarNotificacion(NumOP, NumNotificacion, NumContador, token, trEliminar);
			}
			else {
				//token no es valido: mostramos modal para login
				deleteCookie("token");
...
function esValidoLoginEliminarNotificacion(NumOP, NumNotificacion, NumContador, trEliminar){
	var usuario=$("#txt-usuario2").val();
	var password=$("#txt-password2").val();
	...
	$.ajax({ 
			...
			beforeSend: function (){
				$("#mensaje-notificacion2").addClass("d-none");
	            $("#btn-aceptar-modal-login-eliminar-notificacion").find(".spinner-border").removeClass("d-none");
	            $("#btn-aceptar-modal-login-eliminar-notificacion").attr("disabled","disabled");
	            $("#btn-cancelar-modal-login-eliminar-notificacion").attr("disabled","disabled");
	        }, 
			success: function(data){		
				...
				eliminarNotificacion(NumOP, NumNotificacion, NumContador, data.token, trEliminar);	
			...
			error: function (xhr, ajaxOptions, thrownError) {        	        
				...
				$("#mensaje-notificacion2").removeClass("d-none");
				if(xhr.status==403){
					$("#mensaje-notificacion2").text("No tiene autorización para ejecutar el proceso");
				}
				else {
					$("#mensaje-notificacion2").text("Usuario o Password inválido");
				}
	            
				
	        },
	        complete: function(){
	            $("#btn-aceptar-modal-login-eliminar-notificacion").find(".spinner-border").addClass("d-none");
	            $("#btn-aceptar-modal-login-eliminar-notificacion").removeAttr("disabled");
	            $("#btn-cancelar-modal-login-eliminar-notificacion").removeAttr("disabled");
	        }
		});
}

function eliminarNotificacion(NumOP, NumNotificacion, NumContador, token, trEliminar){
	...
	$.ajax({ 
		...
		beforeSend: function (){
			...
			//mostramos spinner en la fila a eliminar
			trEliminar.find("td.estado .transf-en-progreso").removeClass("d-none");

			//ocultamos icono actual
			trEliminar.find("td.estado i").addClass("d-none");
        }, 
		success: function(data){	
			CodigoRespuesta=data.CodigoRespuesta;
			...
			if(data.CodigoRespuesta.trim()=="0")
			{	
				console.log("se eliminó notificación para NumOP: " + NumOP + ", NumNotificacion: " + NumNotificacion + ", NumContador: " + NumContador);

				//mostramos icono de anulado
				trEliminar.find("td.estado .transf-en-progreso").addClass("d-none");
				trEliminar.find("td.estado i").addClass("d-none");
				trEliminar.find("td.estado .transf-eliminada").removeClass("d-none");

				//seteamos los resultados en los atributos de la fila producto
				trEliminar.attr("data-cod-respuesta",data.CodigoRespuesta.trim());
				trEliminar.attr("data-dsc-respuesta","Se eliminó la notificación con éxito");
				trEliminar.attr("data-estado","A");
				trEliminar.attr("data-numero-notificacion","")
				trEliminar.attr("data-numero-contador","");
				
				trEliminar.attr("data-dsc-respuesta-detalle","");
				if (data.MensajeDetalle!=null){
					trEliminar.attr("data-dsc-respuesta-detalle",data.MensajeDetalle.trim());
				}

				//removemos icono de eliminar notificacion
				trEliminar.find(".eliminar-notificacion").remove();

				//agregamos clase ok a la fila (para que podamos transferirlo)
				//trEliminar.addClass("ok");

				//grabar notificacion en sql
				insertarNotificacionSQL(trEliminar);
			}
			else if (data.CodigoRespuesta.trim()!="0") {

				console.log("se eliminó con errores notificación NumOP: " + NumOP + ", NumNotificacion: " + NumNotificacion + ", NumContador: " + NumContador);

				//mostramos icono de error
				trEliminar.find("td.estado .transf-en-progreso").addClass("d-none");
				trEliminar.find("td.estado i").addClass("d-none");
				trEliminar.find("td.estado .transf-error").removeClass("d-none");

				//seteamos los resultados en los atributos de la fila producto
				trEliminar.attr("data-cod-respuesta",data.CodigoRespuesta.trim());
				trEliminar.attr("data-dsc-respuesta","Error al eliminar la notificación. " + data.Mensaje.trim() );
				trEliminar.attr("data-estado","E");
				trEliminar.attr("data-numero-notificacion","")
				trEliminar.attr("data-numero-contador","");
...
function activarEdicionCelda(objCelda){
	...
	$(document).on("mouseup", function(e){
          ...
          if (!container.is(e.target) && container.has(e.target).length === 0) 
          {
			...
			//seteamos toneladas a cero en caso la fila de producto fue modificada
			$("#cuerpo-tabla-arbol-rdp tr[data-unidad-gas='M3'] td.SUMA_TRANSFERENCIA").text(0);
