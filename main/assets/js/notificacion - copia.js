...
function configuracionEventos(){
	...
	$("#btn-transferir").on("click", function(){
		...
		//mostrarModal("modal-transferir2");
...
function esValidoTransferir(){
	...
	$.ajax({ 
			...
			error: function (xhr, ajaxOptions, thrownError) {        	        
				...
				$("#mensaje-notificacion").text("No tiene permiso para la Notificación del RDP");
	        },
	        complete: function(){
...
function transferir(){
	...
	if ($("tr.transferir.ok").length>0){	
		...
		transferirFila($("tr.transferir.ok")[0]);	

	}	
}

function transferirFila(tr){
	...
	if (NumOP===undefined || NumOP.trim()=="" || NumOP.trim()=="null"){
		...
		if(parseInt(localStorage["nFilas"])<parseInt(localStorage["nFila"])+1){
			...
			transferirFila($("tr.transferir.ok")[parseInt(localStorage["nFila"])]);
		}
...
/*
        		id = idAplicacionFinal + "|notificacion|rdp|" + fecha + "|" + val.COD_GRUPO + "|||";

        		tr = tr + '<tr data-id="' + val.COD_GRUPO + '|" class="arbol-nivel-1">';
        		tr = tr + '		<td class="text-left">';                
                tr = tr + 			 val.COD_GRUPO.trim();
                tr = tr + '		</td>';
                ...
                tr = tr + '		<td class="text-right"></td>';
                tr = tr + '		<td class="text-right TH d-none">' + val.TH + '</td>';
...
/*
        			id = idAplicacionFinal + "|notificacion|rdp|" + fecha + "|" + val2.COD_GRUPO + "|" + val2.COD_MAQUINA + "||";

        			tr = tr + '<tr data-id="' + val2.COD_GRUPO + '|' + val2.COD_MAQUINA + '|" data-padre-id="' + val2.COD_GRUPO + '|" class="arbol-nivel-2">';
        			tr = tr + '		<td class="text-left">'+val2.COD_GRUPO +'</td>';
        			tr = tr + '		<td class="text-left">';                	
                	tr = tr + 			 val2.COD_MAQUINA.trim();
                	tr = tr + '		</td>';                
                	tr = tr + '		<td class="text-left"></td>';
                	tr = tr + '		<td class="text-left"></td>';
                	tr = tr + '		<td class="text-left"></td>';                	
	                ...
	                tr = tr + '		<td class="text-center"></td>';
	                tr = tr + '		<td class="text-right"></td>';
	                tr = tr + '		<td class="text-right TH d-none">' + val2.TH + '</td>';
...
*/
					$.each(val2.PRODUCTOS, function (key3, val3) {	
						...
						$.each(val3.MATERIALES, function (key4, val4) {	
		        			...
		        			tr = tr + '		<td class="text-left">'+val4.COD_GRUPO.trim() +'</td>';
		        			tr = tr + '		<td class="text-left">'+val4.COD_MAQUINA.trim()+'</td>';
		        			tr = tr + '		<td class="text-left">'+val4.COD_PRODUCTO.trim()+'</td>';
							...
							$.each(val4.MATERIALES_SAP, function (key5, val5) {	
								...
								if (!(val5.CNT_PORCENTAJE=="0" || val5.CNT_PORCENTAJE=="")){
									tr = tr + '<tr data-id="' + val4.COD_GRUPO + '|' + val4.COD_MAQUINA + '|' + val4.COD_PRODUCTO + '|' + val4.COD_MATERIAL + '|" data-padre-id="' + val4.COD_GRUPO + '|' + val4.COD_MAQUINA + '|' + val4.COD_PRODUCTO + '|' + val4.COD_MATERIAL + '|" data-unidad="'+val5.COD_UNIDAD+'" class="fila-material-sap" style="background-color:'+color+';">';
				        			tr = tr + '		<td class="text-left">'+val4.COD_GRUPO.trim() +'</td>';
				        			tr = tr + '		<td class="text-left">'+val4.COD_MAQUINA.trim()+'</td>';
				        			tr = tr + '		<td class="text-left">'+val4.COD_PRODUCTO.trim()+'</td>';
				        			tr = tr + '		<td class="text-left">'+val4.COD_MATERIAL.trim()+'</td>';
						    		...
						    		/*
						    		if (primero){
										tr = tr + '			<i class="fas fa-exclamation-triangle" data-cod="-1" data-msj="No se pudo transferir el dato"></i>';
										tr2 = tr2 + '			<i class="fas fa-exclamation-triangle" data-cod="-1" data-msj="No se pudo transferir el dato"></i>';
										primero=false;
						    		}
						    		else {
						    			tr = tr + '			<i class="fas fa-check-double resultado" data-cod="0" data-msj="El dato se transfirió con éxito"></i>';						    			
						    			tr2 = tr2 + '			<i class="fas fa-check-double resultado" data-cod="0" data-msj="El dato se transfirió con éxito"></i>';						    			
						    		}
						    		*/
